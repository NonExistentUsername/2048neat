...
class Game2048:
    ...
    def __check_if_move_legal(self, move) -> bool:
        ...
        return game.__board != self.__board

    @property
    def legal_moves(self) -> list[int]:
        return [move for move in MOVES if self.__check_if_move_legal(move)]

    @property
    def empty_cells(self) -> int:
        return sum(row.count(0) for row in self.__board)

    @property
    def game_end(self) -> bool:
        return not any(self.__check_if_move_legal(move) for move in MOVES)
    ...
    def add_random_tile(self) -> None:
        empty_tiles = [
            (y, x)
            for y, x in itertools.product(range(self.__size_y), range(self.__size_x))
